...
export async function sendRequest(rest, options) {
    try {
        ...
        const response = await dntShim.fetch(options.url.startsWith(BASE_URL) ? options.url : `${BASE_URL}/v${rest.version}/${options.url}`, {
        ...
        if (response.status < 200 || response.status >= 400) {
            ...
            if (rest.invalidRequestErrorStatuses.includes(response.status) &&
                ...
                if (!rest.invalidRequestsTimeoutId) {
                    rest.invalidRequestsTimeoutId = dntShim.setTimeout(() => {
