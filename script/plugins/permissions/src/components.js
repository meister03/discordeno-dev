...
exports.validateComponents = void 0;
...
function validateComponents(bot, components) {
    ...
    for (const component of components) {
        ...
        if (component.components?.length > 5) {
            ...
            component.components.some((subComponent) => subComponent.type === deps_js_1.MessageComponentTypes.SelectMenu)) {
        ...
        for (const subComponent of component.components) {
            ...
            if (subComponent.type === deps_js_1.MessageComponentTypes.Button) {
                if (subComponent.style === deps_js_1.ButtonStyles.Link && subComponent.customId) {
                    throw new Error("Link buttons can not have custom ids.");
                }
                // Other buttons must have a customId
                if (!subComponent.customId && subComponent.style !== deps_js_1.ButtonStyles.Link) {
            ...
            if (subComponent.type === deps_js_1.MessageComponentTypes.SelectMenu) {
