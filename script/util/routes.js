...
const getMessages_js_1 = require("../helpers/messages/getMessages.js");
...
exports.routes = {
    ...
    CHANNEL_MESSAGES: (channelId, options) => {
        ...
        if (options) {
            if ((0, getMessages_js_1.isGetMessagesAfter)(options) && options.after)
                url += `after=${options.after}`;
            if ((0, getMessages_js_1.isGetMessagesBefore)(options) && options.before)
                url += `&before=${options.before}`;
            if ((0, getMessages_js_1.isGetMessagesAround)(options) && options.around)
                url += `&around=${options.around}`;
            if ((0, getMessages_js_1.isGetMessagesLimit)(options) && options.limit)
    ...
    GUILD_BANNER: (guildId, icon) => {
        return `${constants_js_1.baseEndpoints.CDN_URL}/banners/${guildId}/${icon}`;
    },
    ...
    GUILD_ICON: (guildId, icon) => {
        return `${constants_js_1.baseEndpoints.CDN_URL}/icons/${guildId}/${icon}`;
    },
    ...
    GUILD_SPLASH: (guildId, icon) => {
        return `${constants_js_1.baseEndpoints.CDN_URL}/splashes/${guildId}/${icon}`;
    },
    ...
    USER_AVATAR: (userId, icon) => {
        return `${constants_js_1.baseEndpoints.CDN_URL}/avatars/${userId}/${icon}`;
    },
    // TODO: move this away
    USER_DEFAULT_AVATAR: (icon) => {
        return `${constants_js_1.baseEndpoints.CDN_URL}/embed/avatars/${icon}.png`;
    },
    USER_DM: () => {
