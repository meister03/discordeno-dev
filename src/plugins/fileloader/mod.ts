...
export async function importDirectory(path: string) {
  ...
  for (const file of files) {
    ...
    if (file.isFile) {
      ...
      paths.push(
        `import "${dntShim.Deno.mainModule.substring(0, dntShim.Deno.mainModule.lastIndexOf("/"))}/${
...
export async function fileLoader() {
  ...
  await import(
    `${dntShim.Deno.mainModule.substring(0, dntShim.Deno.mainModule.lastIndexOf("/"))}/fileloader.ts#${uniqueFilePathCounter}`
  );
  paths = [];
}
